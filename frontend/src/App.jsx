...
const App = () => {
  ...
  useEffect(() => {
    if (activeTab === 'chat' && chatEndRef.current instanceof Element) {
      try {
        ...
        console.warn("[SCROLL-ERROR] Failed to scroll chat:", e);
      }
    }
  }, [messages, activeTab]);
  ...
  const currentTabContent = () => {
    switch(activeTab) {
      ...
      case 'forge': return <AutonomyPanel globalDirective={globalDirective} autonomyLog={autonomyLog} swarmStatus={swarmStatus} onDeployBrain={handleDeployBrain} onSpawnBrain={spawnBrain} onStopBrain={stopBrain} />;
