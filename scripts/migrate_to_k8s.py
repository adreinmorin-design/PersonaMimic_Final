...
def run_migration():
    print("--- STARTING NEURAL DATA MIGRATION ---")
    ...
    check_pod = subprocess.run(
        ["kubectl", "get", "pod", LOCAL_POD, "-n", LOCAL_NAMESPACE], capture_output=True, text=True
    )
    if "Running" not in check_pod.stdout:
        print(
            f"Error: Local pod {LOCAL_POD} is not running. Please run 'kubectl apply -f k8s/' first."
        )
        sys.exit(1)
