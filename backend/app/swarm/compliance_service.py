...
class ComplianceService:
    ...
    def generate_faq(self, product_name: str, niche: str, specs: str) -> str:
        ...
        res = self.engine.generate_response(prompt, persona_type="mimic")
        content = res.get("content", "# FAQ")
        
        # Ensure it's not wrapped in backticks
        if content.startswith("```"):
            from app.swarm.tools import _extract_code
            content = _extract_code(content)

        return content

    def generate_legal_bundle(self, product_name: str, niche: str) -> dict[str, str]:
        ...
        tos_res = self.engine.generate_response(tos_prompt, persona_type="mimic")
        ...
        pp_res = self.engine.generate_response(pp_prompt, persona_type="mimic")
        bundle["PRIVACY_POLICY.md"] = pp_res.get("content", "# Privacy Policy")

        # Clean all content
        from app.swarm.tools import _extract_code
        for key in bundle:
            if bundle[key].startswith("```"):
                bundle[key] = _extract_code(bundle[key])

        return bundle

    def write_compliance_docs(self, product_name: str, niche: str, specs: str) -> str:
        ...
        try:
            ...
            # FAQ
            faq_content = self.generate_faq(product_name, niche, specs)
            with open(os.path.join(target_dir, "FAQ.md"), "w", encoding="utf-8") as f:
                f.write(faq_content)
            log.append("[OK] FAQ.md generated.")

            # Legal
            legal_bundle = self.generate_legal_bundle(product_name, niche)
